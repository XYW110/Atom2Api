...
func TestSettingsHandlers(t *testing.T) {
	...
	updateRequest := httptest.NewRequest(http.MethodPut, "/api/settings", strings.NewReader(`{"user_agent":"api-client/3.0","admin_password":"updated-secret","audit_debug_enabled":true,"request_retry_count":3,"request_retry_status_codes":"500-502,503,429,500","audit_retention_days":45,"audit_detail_retention_days":15}`))
	...
	if response.UserAgent != "api-client/3.0" || !response.AuditDebugEnabled || response.RequestRetryCount != 3 || response.RetryStatusCodes != "429,500-503" || response.AuditRetentionDays != 45 || response.AuditDetailRetentionDays != 15 {
