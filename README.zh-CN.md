...
模型分流遵循套餐能力：DeepSeek 优先使用未耗尽的 Lite，Lite 不可用后回退到 Pro/Max；GLM-5.2 仅从 Pro、Pro 体验版和 Max 账号中选择；其余模型（包括 Lite 上的 Qwen）按上游 Coding Plan 目录开放。填充模式下，密钥管理页面会显示每个模型的当前账号及其 5 小时调用额度。
