...
import { AlertCircle, Boxes, Braces, Pencil, Search } from 'lucide-react';
...
export default function ModelsPage() {
  ...
  const saveSetting = async (model: ModelRecord, values: { alias: string; enabled: boolean }) => {
    setBusy(model.upstream);
    try {
      await apiFetch('/api/models/settings', jsonRequest('PUT', { upstream: model.upstream, alias: values.alias, enabled: values.enabled }));
      await load();
  ...
  const submitAlias = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (await saveSetting(editing, { alias: alias.trim(), enabled: editing.enabled })) editor.onClose();
  };
  ...
  return (
    <PageShell title="模型管理" description="Coding Plan 模型目录与对外 OpenAI 模型别名">
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-lg border border-zinc-200 bg-white px-4 py-3.5"><p className="text-xs text-zinc-500">已发现模型</p><p className="mt-1 text-xl font-semibold text-zinc-900">{models.length}</p></div>
        ...
        <div className="flex items-center gap-3 rounded-lg border border-zinc-200 bg-white px-4 py-3.5"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-zinc-100 text-zinc-600"><Braces size={18} /></span><div className="min-w-0"><p className="text-xs text-zinc-500">兼容端点</p><code className="mt-1 block truncate font-mono text-sm font-medium text-zinc-800">POST /v1/chat/completions</code></div></div>
      </section>

      <section className="overflow-hidden rounded-lg border border-zinc-200 bg-white">
        <div className="border-b border-zinc-100 p-4"><Input aria-label="搜索模型" className="w-full sm:max-w-xs" classNames={{ inputWrapper: 'h-10 rounded-md border border-zinc-200 bg-white shadow-none' }} placeholder="搜索对外别名或上游模型" radius="sm" startContent={<Search size={16} className="text-zinc-400" />} value={query} variant="bordered" onValueChange={setQuery} /></div>
